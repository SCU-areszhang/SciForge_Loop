...
test('secrets are file references and the offline preflight has no mutation transport', async () => {
  ...
  assert.match(cloud, /SCIFORGE_RUN0_CLOUD_DB_URL_FILE/u)
  assert.match(cloud, /SCIFORGE_RUN0_CLOUD_DB_PGPASS_FILE/u)
  assert.match(cloud, /SCIFORGE_COLLABORATION_DATABASE_URL_FILE: \/run\/secrets\/cloud-db\.url/u)
  assert.doesNotMatch(cloud, /^\s+SCIFORGE_COLLABORATION_DATABASE_URL:/mu)
