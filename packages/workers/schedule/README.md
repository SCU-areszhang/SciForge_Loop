...
```bash
node --import tsx src/cli.ts --base-url http://127.0.0.1:8788
```
...
`GUI_SCHEDULE_INTERNAL_SECRET` is injected only by SciForge's managed Schedule
runtime. The standalone command therefore fails closed unless it is started by
that runtime; the authority is not a supported user setting or CLI argument.
