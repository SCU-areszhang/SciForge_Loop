...
`[I]` 是开发者 A 同时承担的 integration/platform 帽子，不是第三名开发者。`[I]` 独占通用 Host/SDK/Broker/CI/generator/integration files、root lock/toolchain、generated outputs、cross-package harness 和 OpenSpec `tasks.md` checkbox。

你不得领取 `[A]` 或 `[I]` task，不得修改另一 Owner 的领域语义。Review 不转移 authorship 或 path ownership。
