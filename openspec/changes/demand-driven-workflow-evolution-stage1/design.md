...
There are only two developers. For all of Stage1, `[I]` is Developer A's integration/platform hat, not a third person and not a swappable owner. Developer B remains inside the agreed B scope. The integration hat works on `stage1/i-*` branches and alone updates:
